...
- Is the dream feed additive, or does it re-phase the surrounding cascade? (Note: §F73 removed the `bottlesPerDay` daily-count budget, so the old "counts toward the total" framing is moot — the cascade fills the day regardless.)
