...
type InternalForm = {
  ...
  label: string;
  // Pump per-side volume, ounces. Default 0; assembled into pumpVolumeOz on save.
  pumpLeftOz: number;
  pumpRightOz: number;
};
...
function eventToForm(event: Event | null): InternalForm {
  return {
    ...
    label: event?.label ?? "",
    pumpLeftOz: event?.pumpVolumeOz?.left ?? 0,
    pumpRightOz: event?.pumpVolumeOz?.right ?? 0,
  };
}
...
function confirmCopyFor(action: DrawerDestructiveAction, event: Event) {
  if (action === "reset") {
    return {
  ...
  if (event.type === "daily_recurring") {
    return {
      title: `Skip ${event.label} today?`,
      body: "It'll come back tomorrow.",
      confirmLabel: "Skip today",
    };
  }
  return {
    title: `Delete this ${event.type === "extra" ? "event" : event.type.replace("_", " ")}?`,
...
export function EventEditDrawerV3({
  ...
  // Dream feed is excluded from isNearestBottle (not in the rhythm chain) but is always its own nearest.
  ...
  const handleSave = () => {
    ...
    const formForTransform: FormState = {
      ...
      owner: form.owner,
      label: form.label,
      ...(showVolumes ? { pumpVolumeOz: { left: form.pumpLeftOz, right: form.pumpRightOz } } : {}),
    };
