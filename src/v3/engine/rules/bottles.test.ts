...
describe("§F66 fast-follow B10 — suppressedBottleEventKeys prevents re-emission", () => {
  it("a suppressed cold-start slot is dropped from output and not re-numbered", () => {
    // Day.suppressedBottleEventKeys = ["bottle_2"]: simulates the
    // user deleting an auto-promote-persisted bottle_2 ghost. The
    // cascade walks the full chain so subsequent slots land at their
    // natural times, but bottle_2 is filtered before output and
    // R5.4 won't re-assign that number to a different projection.
    const ctx = aContext({
      day: aDay({ wakeTime: 7 * 60, suppressedBottleEventKeys: ["bottle_2"] }),
      settings: aSettings({
        bottleChain: { bottlesPerDay: 4, bufferAfterWakeMinutes: 10 },
        defaultBottleIntervalMinutes: 180,
      }),
      actuals: [],
      nowMinutes: 6 * 60, // before any cascade slot — keep all projected
    });
    const out = projectDay(
      {
        day: ctx.day,
    ...
    const bottles = out
      .filter((e) => e.type === "bottle")
      .sort((a, b) => a.startTime - b.startTime);
    // bottle_2 must not appear, and no other projection should be
    // renumbered to "bottle_2" either.
    expect(bottles.find((b) => b.eventKey === "bottle_2")).toBeUndefined();
    // The remaining projections keep their natural times: 7:10,
    // 13:10, 16:10 (slot 2's 10:10 is the suppressed gap).
    expect(bottles.map((b) => b.startTime)).toEqual([7 * 60 + 10, 13 * 60 + 10, 16 * 60 + 10]);
  });

  it("idempotent: a second engine pass on the same suppressed-day input produces the same output", () => {
    // Catches the canCascade loop bug: if effTarget isn't subtracted,
    // pass 2 sees chainBottles.length < target and re-emits, looping.
    const ctx = aContext({
      day: aDay({ wakeTime: 7 * 60, suppressedBottleEventKeys: ["bottle_2"] }),
      settings: aSettings({
        bottleChain: { bottlesPerDay: 4, bufferAfterWakeMinutes: 10 },
